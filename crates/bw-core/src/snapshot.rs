...
pub struct SnapshotSizing {
    ...
    /// Deprecated multiplier alias for percentage / 100, greater than zero and at most 2.
    pub scale: Option<f64>,
}

impl SnapshotSizing {
    pub fn validate(self) -> Result<(), &'static str> {
        if [self.width, self.height, self.percentage, self.scale].iter().flatten().count() > 1 {
            return Err("supply only one of width, height, percentage, or scale");
        }
        ...
        for (value, limit) in [(self.percentage, 200.0), (self.scale, 2.0)] {
            if value.is_some_and(|v| !v.is_finite() || v <= 0.0 || v > limit) {
                return Err("percentage must be greater than 0 and at most 200; legacy scale at most 2");
            }
        }
        Ok(())
    }
    ...
    pub fn resolve(self, native_width: f64, native_height: f64) -> Result<(i32, i32, f64), &'static str> {
        ...
        let ratio = if let Some(width) = self.width { width / native_width }
            ...
            else { self.percentage.map(|p| p / 100.0).or(self.scale).unwrap_or(1.0) };
...
mod tests {
    ...
    fn sizing_and_limits() {
        ...
        assert!(SnapshotSizing { width: Some(64.0), scale: Some(1.0), ..Default::default() }.validate().is_err());
