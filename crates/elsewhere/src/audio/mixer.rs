...
use elsewhere_core::audio::{Command, Event, Kind, Level, Node, Request, Snapshot};
...
pub struct Requests {
    pub epoch: Option<Arc<elsewhere_core::audio::Epoch>>,
...
impl Record {
    fn kind(&self) -> Option<Kind> {
        ...
        if name.starts_with(meter::NAME) || matches!(name, "elsewhere-microphone-input" | "elsewhere-capture" | "elsewhere-microphone-stream") { return None; }
...
fn connect(remote: &Path, requests: Rc<Receiver<Request>>, audience: Audience, shared_epoch: Option<Arc<elsewhere_core::audio::Epoch>>, events: tokio::sync::mpsc::Sender<Event>, stopping: Arc<AtomicBool>) -> Result<()> {
    ...
    let core = context.connect_fd_rc(UnixStream::connect(remote)?.into(), Some(pw::properties::properties! { "application.name" => "Elsewhere mixer", "application.id" => "elsewhere-mixer" }))?;
