...
python /src/crates/bw/checks/audio-lifecycle.py /src/target/release/browser-wayland
"""
import os
...
def owned_audio(pid):
    ...
    for proc in Path("/proc").iterdir():
        ...
        try:
            ...
            if root.startswith("/tmp/bw-audio-"):
...
def run_case(name, *, replacement=None, no_audio=False, during_startup=False, group_signal=False, kill_service=None, missing_plugin=False, missing_home=False, freeze_worker=False):
    with tempfile.TemporaryDirectory(prefix="bw-audio-check-") as directory:
        root = Path(directory)
        ...
        try:
            ...
            if process.returncode != 0:
                Path("/tmp/bw-audio-lifecycle-failure.log").write_text((root / "log").read_text())
            assert process.returncode == 0, f"{name}: exit {process.returncode}, see /tmp/bw-audio-lifecycle-failure.log"
