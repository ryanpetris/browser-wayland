...
const disabled = process.env.BW_VISUALISER === '0';
const dist = new URL('../dist/', import.meta.url);
const files = await readdir(dist, { recursive: true });
if (disabled) {
  assert(!files.some(f => /visualiser|audiomotion|viewer-source/.test(f)), files.join('\n'));
  assert(!/audiomotion|visualiser-/i.test(await readFile(new URL('app.js', dist), 'utf8')));
}
...
try {
  ...
  if (!disabled) await page.route('**/assets/visualiser-*.js', async route => {
  ...
  if (disabled) {
    ...
    assert.equal(errors.length, 0, errors.join('\n'));
    console.log('enabled: renderer failure isolation, lazy load, styles, HiDPI, fullscreen, animation off, reduced motion, hidden page/stage, close while loading, eight open/close cycles, listener and graph ownership, silence, source replacement passed');
  }
