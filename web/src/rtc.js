...
export function openRtc({ iceServers, signal, onMessage, onOpen, onClose }) {
  ...
  ch.onmessage = e => {
    ...
    if (dv.getUint8(0) !== FRAGMENT) { onMessage(e.data); return; }
  ...
  // one within the wait): the server answers with its own and does no trickle
  let offered = false;
  const offer = () => { if (!offered && pc.localDescription) { offered = true; signal({ offer: pc.localDescription.sdp }); } };
  pc.onicegatheringstatechange = () => { if (pc.iceGatheringState === 'complete') offer(); };
  pc.createOffer().then(o => pc.setLocalDescription(o)).then(() => setTimeout(offer, 1500)).catch(onClose);
  return {
