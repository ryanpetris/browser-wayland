...
| `0x0C` | StreamState | JSON `{"codec", "auto_codec", "preset", "ceiling_kbps", "medium_kbps", "bitrate_kbps", "max_fps"}`: selected preset and ceiling, configured Medium ceiling, and current encoder target (`max_fps` 0: the compositor's rate), after every `Config` and whenever the rate controller steps the quality; the page separates ceiling, encoder target, and measured throughput. |
| `0x0D` | Rtc | JSON, WebRTC signalling: `{"ice_servers": [...]}` right after the replay when the server does WebRTC (the page may then offer); `{"answer": "<sdp>", "g": 1}` to the page's offer; `{"close": true, "g": 1}` when the server gives the channel up (a frame three seconds at the front of its queue): the video is already back on the socket, the page closes its side and retries while keeping WebRTC selected. A rejected offer returns the same close envelope with `"reason": "Offer rejected"`. `{"keyframe": true, "g": 1}` acknowledges a matching close that released a video claim and precedes a server-requested refresh; the page expects a keyframe even if its channel-open callback never ran. |
...
| `0x81` | Hello | `u8 hw` `u8 sw`: codec families the browser decodes with hardware, and at all (bit 0 H.264, bit 1 HEVC, bit 2 VP9, bit 3 AV1, bit 4 VP8); then, optionally, `u8 codec` (0 Auto, else 1 H.264, 2 HEVC, 3 VP9, 4 AV1, 5 VP8) and `u8 quality` (1 very-low, 2 low, 3 medium, 4 high, 5 max; absent or invalid means max), this viewer's choice. Picks the codec, among those this machine encodes, and starts the stream. |
...
| `0x8F` | Stream | JSON `{"codec": "auto" \| "h264" \| "hevc" \| "vp9" \| "av1" \| "vp8", "quality": "very-low" \| "low" \| "medium" \| "high" \| "max"}`, either field optional: this session's choice, applied live (a codec change restarts the stream with a new id; a codec the browser or the server can't do means Auto). Any session, its own stream only. |
...
The Docker rig can run `node web/checks/rtc-peer.mjs` for peer disposal and signaling callbacks, and `node web/checks/quality.mjs` for live desktop/window transport and quality. `node web/checks/rtc-recovery.mjs` exercises real channels, fallback, retry and cancellation; run that check with NET_ADMIN inside Docker so it can temporarily block its test server’s UDP port. It removes its dedicated firewall chain on exit.
