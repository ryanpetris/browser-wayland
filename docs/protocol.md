...
| `0x0D` | Rtc | JSON, WebRTC signalling: `{"ice_servers": [...]}` right after the replay when the server does WebRTC (the page may then offer); `{"answer": "<sdp>"}` to the page's offer; `{"close": true}` when the server gives the channel up (a frame stuck at the front of its queue for three seconds): the video is already back on the socket, the page closes its side and its pick follows. |
| `0x0E` | Fragment | On the data channel only: `u32 id` `u16 index` `u16 count` then a piece of one message (a video frame; 16 kB pieces), reassembled by id. The channel is unordered but reliable, so a piece is delayed rather than lost; a frame the server dropped (its queue full, or a keyframe replacing what waited) shows as a seq gap and gets a keyframe. |
