...
| `0x95` | Rtc | JSON: `{"offer": "<sdp>"}` from the page to open its `video` data channel (unordered, no retransmits; the page sends the offer once its candidates are gathered, the server answers as an ICE-lite peer with its addresses); `{"close": true}` to go back to the socket. While a session's channel is open its video frames go there instead of the socket, with the same framing; everything else stays on the socket. |
