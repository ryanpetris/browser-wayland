...
| `0x08` | Role | `u8 role` `u8 microphone`: what this session may do. 0 watch only (the viewer token); 1 act but not drive (a control token while another session controls); 2 control: its pointer, keyboard and window size are the desktop's. `microphone` is 1 when the desktop takes the browser's microphone (`Mic`). Sent with the replay after `Hello` and whenever it changes. |
