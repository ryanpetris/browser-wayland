...
| `0x81` | Hello | `u8 hw` `u8 sw`: codec families the browser decodes with hardware, and at all (bit 0 H.264, bit 1 HEVC, bit 2 VP9, bit 3 AV1, bit 4 VP8); then `u8 codec` (0 Auto, else 1 H.264, 2 HEVC, 3 VP9, 4 AV1, 5 VP8) and `u8 quality` (1 very-low, 2 low, 3 medium, 4 high, 5 max; invalid means max), this viewer's choice. Picks the codec, among those this machine encodes, and starts the stream. |
