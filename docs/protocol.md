...
| `0x08` | Role | `u8 role` `u8 features`: what this session may do. 0 watch only (the viewer token); 1 act but not drive (a control token while another session controls); 2 control: its pointer, keyboard and window size are the desktop's. `features` says what the desktop takes from the browser: bit 0 its microphone (`Mic`), bit 1 its webcam (`Cam`). Sent with the replay after `Hello` and whenever it changes. |
| `0x09` | Notice | UTF-8 text about the session's last action, for the page to show briefly. Sent to the controller when its drag (`Drag`) was dropped: whether the application took the files. Sent to a window stream whose press aims past the desktop's edge at an X11 window: Xwayland's screen is the desktop, and the X server pins the pointer to it, so that click cannot arrive. |
...
| `0x0C` | StreamState | JSON `{"codec", "auto_codec", "bitrate_kbps", "max_fps"}`: what this session's encoder does right now (`max_fps` 0: the compositor's rate), after every `Config` and whenever the rate controller steps the quality; the page labels its quality entries with it. |
| `0x0D` | Rtc | JSON, WebRTC signalling: `{"ice_servers": [...]}` right after the replay when the server does WebRTC (the page may then offer); `{"answer": "<sdp>"}` to the page's offer; `{"close": true}` when the server gives the channel up (a frame three seconds at the front of its queue): the video is already back on the socket, the page closes its side and its pick follows. |
...
| `0x81` | Hello | `u8 hw` `u8 sw`: codec families the browser decodes with hardware, and at all (bit 0 H.264, bit 1 HEVC, bit 2 VP9, bit 3 AV1, bit 4 VP8); then, optionally, `u8 codec` (0 Auto, else 1 H.264, 2 HEVC, 3 VP9, 4 AV1, 5 VP8) and `u8 quality` (0 Auto, 1 low, 2 medium, 3 high, 4 max), this viewer's choice. Picks the codec, among those this machine encodes, and starts the stream. |
...
| `0x95` | Rtc | JSON: `{"offer": "<sdp>"}` from the page to open its `video` data channel (ordered, reliable; the page sends the offer once its candidates are gathered, the server answers as an ICE-lite peer with its addresses); `{"close": true}` to go back to the socket. `g` numbers the attempt and comes back with the answer. While a session's channel is open its video frames go there instead of the socket, with the same framing; everything else stays on the socket. Desktop and window sessions alike; the page offers only when a viewer picks the channel. |
...
| `PUT /api/drop/{name}` | Like `PUT /api/files/{name}`, but into the batch a drag or a paste is staging for the application that will take the files (a randomly named directory under the cache directory, closed by the `Drag` `drop` or by `POST /api/clipboard/files` with `"staged": true`; swept after a day, sent to the transfer folder if nothing took it). `201` with `{"name": "…"}`. |
...
| `POST /api/clipboard/files` | Body `{"names": [...]}`, files of the transfer folder, or with `"staged": true` of the batch `PUT /api/drop/{name}` staged: they become the desktop clipboard as a file manager's copy (`text/uri-list` and `x-special/gnome-copied-files`). `202`; `404` with nothing staged. |
